process BCFTOOLS_CONCAT {
    ...
    for f in ${input}; do
        if bcftools view -H "\$f" | head -n1 >/dev/null 2>&1; then
            valid_vcfs+=( "\$f" )
        fi
    done

    # If no valid VCFs, create empty output with header and exit cleanly
    if [ "\${#valid_vcfs[@]}" -eq 0 ]; then
        echo "## No valid VCFs found, creating empty VCF" >&2
        bcftools view -h ${vcfs[0]} > ${prefix}.${extension}
        bgzip -f ${prefix}.${extension} || true
        tabix -f -p vcf ${prefix}.${extension}.gz || true
        exit 0
    fi

    # --------------------------------------------------
    # Concat → SNP filter → sort
    # --------------------------------------------------
    bcftools concat \\
        --output tmp.${prefix}.${extension} \\
        $args \\
        --threads $task.cpus \\
        "\${valid_vcfs[@]}"
	...
	--include 'TYPE="snp"' \\
        $args \\
        --threads $task.cpus \\
        --output tmp2.${prefix}.${extension} \\
        tmp.${prefix}.${extension}

    bcftools sort \\
	$args \\
	-o ${prefix}.${extension} \\
	tmp2.${prefix}.${extension}
