...
include { QUANTIFY_SCRNA_SNP      } from '../../subworkflows/local/quantify_scrna_snp.nf'
include { QUANTIFY_SCRNA_ASE      } from '../../subworkflows/local/quantify_scrna_ase.nf'

// MODULES
include { ISOSCELES                                     } from '../../modules/local/isosceles.nf'
...
include { IRESCUE                                       } from '../../modules/local/irescue.nf'
...
workflow PROCESS_LONGREAD_SCRNA {
	...
	if (quant_list.contains("isosceles")) {
    	    bam_meta = ch_bam.map{ m, b -> m }
            fasta_fixed = bam_meta.combine(fasta.map{m, fa -> fa}).map{ id, fa ->
                [ [id:id.id], fa ]
            }
            fai_fixed = bam_meta.combine(fai.map{m, fai -> fai}).map{ id, fi ->
                [ [id:id.id], fi ]
            }

            bam_bai = ch_bam.join(ch_bai, by: [0])
            bam_bai_fa = bam_bai.join(fasta_fixed, by: [0])
            isoform_input_ch = bam_bai_fa.join(fai_fixed, by: [0])
	    ISOSCELES(isoform_input_ch)
	}
	
	if (quant_list.contains("TE"))  {
	    bam_bai = ch_bam.join(ch_bai, by: [0])
	    IRESCUE(bam_bai)	
	}
	
	if (quant_list.contains("ASE"))  {
	    bam_meta = ch_bam.map{ m, b -> m }
            fasta_fixed = bam_meta.combine(fasta.map{m, fa -> fa}).map{ id, fa ->
                [ [id:id.id], fa ]
            }
            ...
            bam_bai = ch_bam.join(ch_bai, by: [0])
            bam_bai_fa = bam_bai.join(fasta_fixed, by: [0])
            ase_input_ch = bam_bai_fa.join(fai_fixed, by: [0])
	    QUANTIFY_SCRNA_ASE(ase_input_ch)
        }

	if (quant_list.contains("SNP")) {
	    ...
	    bam_bai = ch_bam.join(ch_bai, by: [0])
    	    bam_bai_fa = bam_bai.join(fasta_fixed, by: [0])
    	    snp_input_ch = bam_bai_fa.join(fai_fixed, by: [0])
	    QUANTIFY_SCRNA_SNP(snp_input_ch)
	}	
